...
export async function serviceCommand(serviceIds, useExitCode, detail) {
...
function printCompactLine(svc, fmt) {
...
function printDetailCard(svc, fmt) {
    const label = svc.status.replace(/_/g, " ");
    console.log();
    console.log(boxTop(WIDTH));
    console.log(boxLine(`${fmt.color(fmt.symbol)} ${chalk.bold(svc.name || svc.id)}  ${fmt.color(label)}`, WIDTH));
    if (svc.description) {
    ...
    if (svc.components && svc.components.length > 0) {
        ...
        for (const comp of svc.components) {
            ...
            console.log(boxLine(`  ${cfmt.color(cfmt.symbol)} ${comp.name}`, WIDTH));
        }
    }
    if (svc.incidents && svc.incidents.length > 0) {
        ...
        for (const inc of svc.incidents.slice(0, 3)) {
            ...
            console.log(boxLine(`  ${chalk.yellow("›")} ${inc.title}`, WIDTH));
            console.log(boxLine(`    ${chalk.dim(inc.status)} ${chalk.dim("·")} ${chalk.dim(age)}`, WIDTH));
        }
    }
