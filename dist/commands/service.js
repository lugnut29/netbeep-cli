...
export async function serviceCommand(serviceIds, useExitCode, detail, componentFilter) {
...
function printDetailCard(svc, fmt, componentFilter) {
    const label = svc.status.replace(/_/g, " ");
    console.log();
    ...
    if (respLabel)
        parts.push(respLabel);
    if (svc.lastChecked)
        parts.push(chalk.dim("Checked " + timeSince(svc.lastChecked)));
    if (parts.length > 0)
    ...
    if (svc.components && svc.components.length > 0) {
        ...
        if (comps.length > 0) {
            ...
            if (componentFilter) {
                ...
                const otherIssues = svc.components.filter((c) => c.status !== "operational" && !comps.includes(c));
