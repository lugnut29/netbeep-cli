...
const STATUS_MAP: Record<string, { symbol: string; color: (s: string) => string; exit: number }> = {
...
export function getFormat(status: string) {
  return STATUS_MAP[status] ?? STATUS_MAP.unknown;
}

export function formatServiceLine(id: string, status: string, description?: string): string {
